...
def build_pinned_pipeline_workflow():
    task_times_s = []
...
def build_multi_stage_ordered_workflow():
    task_times_s = []
    with open("./data/prio_runtimes.txt", "r") as f:
        task_times_s = [
...
def main(policy: Literal["pinned", "ordered"] = "pinned"):
    ## Build workflow
    if policy == "pinned":
        wf = build_pinned_pipeline_workflow()
    elif policy == "ordered":
        wf = build_multi_stage_ordered_workflow()
    else:
        raise ValueError
    ...
    result = sim.run(
        telemetry=True, fname=f"{os.getcwd()}/telemetry_{policy}.csv", freq=100
    )
...
if __name__ == "__main__":
    main(policy=input("policy[pinned or ordered]:"))
